...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@-----------------------ROW BY ROW SAVE WITH UNIQUE IDENTIFIER IF FAILS SKIPS THE OLD ONE WITHIN THAT DATE-------------------------@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#         # ===== GET DESCRIPTION & LOOKUP FROM LIST (BEFORE CLICK) =====

#         desc_list_xpath = f"/html/body/root/app/program/div/div[2]/div[2]/div/div/screen/div/div/div/proxy[3]/div/asi-panel/div/proxy[3]/div/asi-frame/div/div[2]/proxy/div/asi-frame/div/div[2]/proxy[3]/div/asi-virtual-list-view/div/div/asi-virtual-list-table/div/div/div[2]/div[2]/div/div[{row}]/div[1]/div/span"

#         lookup_list_xpath = f"/html/body/root/app/program/div/div[2]/div[2]/div/div/screen/div/div/div/proxy[3]/div/asi-panel/div/proxy[3]/div/asi-frame/div/div[2]/proxy/div/asi-frame/div/div[2]/proxy[3]/div/asi-virtual-list-view/div/div/asi-virtual-list-table/div/div/div[2]/div[2]/div/div[{row}]/div[2]/div/span"

#         desc_list_ele = page.locator(f"xpath={desc_list_xpath}")
#         lookup_list_ele = page.locator(f"xpath={lookup_list_xpath}")

#         await desc_list_ele.wait_for(timeout=5000)
#         await lookup_list_ele.wait_for(timeout=5000)

#         desc_list_value = (await desc_list_ele.inner_text()).strip()
#         lookup_list_value = (await lookup_list_ele.inner_text()).strip()

#         clean_desc = re.sub(r'\s+', ' ', desc_list_value.lower())
#         unique_id = f"{clean_desc}_{lookup_list_value}"

#         print("CHECK UNIQUE:", unique_id)

#         # ===== SKIP IF ALREADY PROCESSED =====
#         if unique_id in existing_ids:
#             print("Skipping row:", unique_id)
#             row += 1
#             continue
...
#         # existing_ids = set(
#         #     df["Description"].astype(str).str.strip().str.lower().apply(lambda x: re.sub(r'\s+', ' ', x))+ "_" +
#         #     df["Lookup Code"].astype(str)
#         # )
...
#         existing_ids = set(
#             df_filtered["clean_desc"] + "_" +
#             df_filtered["Lookup Code"].astype(str)
#         )
...
# ###########################    Date Condition in RunEpic function for UI Date and Excel Filter Date    ##########################
...
###########################    Latest Browser and Page Retry if it fails  ##########################
...
#         #     print(f"\n Browser failed: {error_text}")

#         #     if "Target closed" in error_text or "Page closed" in error_text:
...
##########################################################    Creating checkpoint file to save progress and avoid duplicates in case of crash  ##########################################################
import asyncio
from playwright.async_api import async_playwright
from configparser import ConfigParser
from datetime import datetime, timedelta
